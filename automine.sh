...
#SCRIPTDIR=../bio/scripts/medicmine

ARKDIR=/micklem/releases/medicmine

RECIPIENTS=contrino@flymine.org

# set minedir and check that medicmine in path
MINEDIR=$PWD
...
PREP4FULL=n      # don't run get_all_medicmine (only in F mode)
...
$progname [-F] [-M] [-R] [-V] [-P] [-L] [-I] [-f file_name] [-g] [-i] [-r release] [-s] [-v] DCCid
  -F: full (medicmine) rebuild (Uses medicmine-build as default)
  -M: test build (metadata only)
  -R: restart full build after failure
  -V: validation mode: all new entries,one at the time (Uses medicmine-val as default)
...
Advanced Usage switches: [-a] [-b] [-p] [-t] [-w] [-x]
  ...
  -p: prepare chadoxml directories and update the sources (run get_all_medicmine). Valid only in F mode
  -t: no acceptance test run
  -w: no new webapp will be built
  -x: don't build medicmine (!: used for running only tests)
...
Notes: The file is downloaded only if not present or the remote copy
       ...
       (default modchado-dev) are ADDED to the relevant medicmine (default: medicmine-dev)

       If no uppercase switch is used (V, M, F, R, P, L, I), the local project.xml is used.

examples:

$progname     add new submissions to medicmine-dev, getting new files from ftp
$progname 123 add submission 123 to medicmine-dev, getting it from ftp
$progname -F -r test    build a medicmine-test with metadata, Flybase and Wormbase,
        getting new files from ftp
$progname -M -r test    build a new chado with all the NEW submissions in
        $FTPURL
        and use this to build a medicmine-test
$progname -s -w -t build medicmine-dev using the existing modchado-dev,
        without performing acceptance tests and without building the webapp
$progname -f file_name -r val build medicmine-val using the submissions listed in file_name
...
#  p )  echo "- prepare directories for full realease and update all sources (get_all_medicmine is run)" ; PREP4FULL=y;;
...
#if [ -n "$P" ]
#then
#SOURCES=medicmine-static,modencode-"$P"-metadata
#elif [ -n "$L" ]
#then
#SOURCES=medicmine-static
#IFS=$','
#for p in $L
#do
#echo "---> $p"
...
#ant -v -Daction=medicmine-metadata-cache -Drelease=$REL\
...
#then
#cd ../bio/scripts
#./get_all_medicmine.sh|| { printf "%b" "\n ThaleMine build (get_all_medicmine.sh) FAILED.\n" ; exit 1 ; }
#fi
# .. and build medicmine
cd $MINEDIR
